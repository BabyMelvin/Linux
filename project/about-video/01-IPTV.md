数字视频和高速宽带网络的应用，在宽带网络上传输实时音频和视频技术叫做: `IPTV(Internet Protocal TV)`、流媒体、`Video Over Ip`和`IP视频`.该技术并不是通过下载和存储大量的视频和音频文件然后播放，而是数据在网络上以流的形式发送。**流将视频和音频数据分割成适合在网络上传播小的数据包**。
