...
有了这两个属性，子节点中的`reg`就可以描述**一块连续的地址区域**。下例中，父节点中指定了`#address-cells = <2>;#size-cells = <1>，`则子节点`dev-bootscs0`中的reg中的前两个数表示一个地址,即`MBUS_ID(0xf0, 0x01)`和`0x1045C`，最后一个数的表示地址跨度，即是`0x4`
