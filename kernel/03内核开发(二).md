...
```
* size:分配内存大小
* flags：分配标志，控制kmalloc行为.
	* GFP_KERNEL:内存分配由运行在内核态`进程`调用的。属于某个`进程`，当空闲内存太少时，kmalloc函数使`当前进程进入睡眠`，等待空闲页出现。
...
#### 直接内存映射区
从3G开始，最大的896M线性地址空间，称为直接内存映射区。这是因为该区域线性和物理地址之间存在线性转换关系`线性地址=3G+物理地址`
物理地址区间0x100000-0x200000 映射到线性空间就是3G+0x100000-3G+0x200000
...
* 使用alloc_page_(__GFP_HIGHMEM)分配高端内存页
...
```
unsinged long j=jiffies+jit_delay*HZ;
while(jiffies<j){
	/*do nothing*/
}
```
